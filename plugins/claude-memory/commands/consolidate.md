...
The consolidation script is:
    ...
    ${CLAUDE_PLUGIN_ROOT}/bin/memory-consolidate.sh

All the deterministic work lives in that bash script — your only job is to launch it without
flooding this session. Spawn a **background subagent** (Task tool, `run_in_background: true`)
with a clean context and give it this task, substituting the script path above for `<SCRIPT>`
and the user's argument string for `<ARGS>` (the raw `$ARGUMENTS` for this command — may be empty):
...
>
> If the script path does not exist, say so and stop — do not search for it.
>
